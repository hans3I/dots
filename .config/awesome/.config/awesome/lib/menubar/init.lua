...
menubar.geometry = { width = screen[1].geometry.width / 0.924 - 150,
                     height = 24,
                     x = screen[1].geometry.width / 8.8 - 150,
                     y = 10 }
...
local list_interspace = theme.xresources.apply_dpi(0) * 3
...
local function prompt_keypressed_callback(mod, key, comm)
    if key == "Left" or (mod.Mod1 and key == "j") then
        current_item = math.max(current_item - 1, 1)
        return true
    elseif key == "Right" or (mod.Mod1 and key == "k") then
